...
@section('subheading')
    Imprime o descarga el comprobante de la orden de servicio.
@endsection

@section('top-actions')
    <a href="{{ route('admin.orders.show', $order) }}" class="btn btn-secondary">← Volver</a>
...
@section('content')
    <div class="panel" id="invoiceContent">
        ...
        <div class="mb-3">
            <p><strong>Descripción del servicio:</strong> {{ $order->description }}</p>
            <p><strong>Programa de trabajo:</strong> {{ $order->scheduled_at?->format('d/m/Y H:i') ?? '—' }}</p>
